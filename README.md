...
zipでダウンロードした場合、Windows がファイルに「インターネットから入手したもの」という
印を付け、PowerShell がスクリプトの実行を止めることがあります

内容を確認したうえで必要な場合だけ、以下のコマンドで実行できます
...
新しいzipをダウンロードした場合は、以下の手順でスクリプトを更新できます。
...
`Clean.ps1` は PowerShell の `$PROFILE` からこのツールの読み込み設定だけを削除します。  
その後 `Setup.ps1` を実行すると、セットアップ済みフォルダを再利用し、`scripts` と `Clean.ps1` とプロンプトファイルを新しい内容で上書きします。

作成済みの録画ファイル、音声ファイル、SRTファイル、切り出し動画は削除しません。
...
SRTファイル名には、文字起こしに使った音声トラック番号が含まれます。  
例: `2026-06-16_午前配信.track1.srt`

LLMが出力する切り出しコマンドでは、`.track1` を除いた同名の管理動画を使います。  
例: `2026-06-16_午前配信.mp4`
...
デフォルトでは、録画ファイルの1番目の音声トラックを配信者の声として扱います。

```powershell
New-SuggestSource -VoiceTrack 1
```
...
- セットアップ後、ダウンロードしたzipの展開フォルダは移動・削除してもかまいません
- 現状、セットアップ先フォルダを後から移動することはできません
  - フォルダを移動したい場合は、セットアップ先の `Clean.ps1` を実行し、移動後の場所で `Setup.ps1` を実行し直してください
  - 保存先フォルダを変更したい場合は、`Clean.ps1` の後に上級者設定で `Setup.ps1` を実行してください
  - `$PROFILE` を手作業で編集することでも対応できますが、初心者にはおすすめしません
